...
- **Verified on the live production voice (2026-09-22).** `x-ai/grok-voice-tts-1.0` / `ara` answered HTTP 200 for a tagged German line, and the resulting audio transcribed (through our own whisper endpoint) to exactly the same sentence as the untagged control — the tag words are not in the audio — while running 7.5s against the control's 6.2s, i.e. the laugh and the whispered clause are really performed.
